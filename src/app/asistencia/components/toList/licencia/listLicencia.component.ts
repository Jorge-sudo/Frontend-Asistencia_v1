...
export class ListLicenciaComponent implements OnInit {
  selectionsData: any[] = [{ name: 'Vigentes' }, { name: 'Antiguos' }];
  ...
  @ViewChild('selectStatus') elementSelectStatus!: Dropdown;

  constructor(
    public docenteLicenciaService: DocenteLicenciaService,
  ...
  loadDataGlobalFilter(): Observable<any> {
    return this.docenteLicenciaService
      .getDocenteLicenciasFilterGlobal(
        this.order,
        this.page,
        this.rows,
        this.sortField,
  ...
  clear(table: Table) {
    ...
    this.elementSelectStatus.writeValue(null);
    table.clear();
  }
}
